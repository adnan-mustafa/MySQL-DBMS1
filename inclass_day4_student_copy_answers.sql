...
select bc.customer_id, bc.customer_name, avg(bad.balance_amount) average_account_balance
...
case
	when bad.account_type='Credit Card' and bad.balance_amount=0 then bat.transaction_amount
...
left join bank_account_transaction bat
on bad.Account_Number = bat.Account_Number
where date_format(bat.Transaction_Date,'%Y-%m') in ('2020-03','2020-04')
order by bat.Transaction_Date
...
on bad.Account_Number=bat.Account_Number
where date_format(bat.Transaction_Date, '%Y-%m') <> '2020-03'
order by bat.Transaction_Date
...
left join bank_account_transaction bat
on bad.Account_Number=bat.Account_Number
where date_format(bat.Transaction_Date,'%Y-%m-%d')<='2020-03-18' and bat.Transaction_amount is NOT NULL
order by bat.Transaction_Date

#To find first quarter:
#select makedate(2020,(max(transaction_date) - min(transaction_date))/4) from bank_account_transaction
...
select bad.Account_Number, bcm.Event, bcm.Customer_message
from bank_account_details bad
cross join bank_customer_messages bcm
where bad.Account_type='Savings' and bcm.Event='Adhoc'
...
case
	when bat.Transaction_amount<0 then bad.Balance_amount+bat.Transaction_amount
    else bad.Balance_amount
end deducted_balance_amount
from bank_account_details bad
left join bank_account_transaction bat
on bad.Account_Number=bat.Account_Number
...
select bad.Account_Number as p_account_number, bad.Account_type as p_account_type, bard.Account_Number as s_account_number, bard.Account_type as s_account_type, bad.Balance_amount Balance_Amount
from bank_account_details bad
left join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
group by p_account_number
...
select bad.Account_Number as p_account_number, bad.Account_type as p_account_type, bard.Account_Number as s_account_number, bard.Account_type as s_account_type, bat.Transaction_amount
from bank_account_details bad
left join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
join bank_account_transaction bat
...
select bad.Account_Number as p_account_number, bad.Account_type as p_account_type, bard.Account_Number as s_account_number, bard.Account_type as s_account_type
from bank_account_details bad
left join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
...
select bad.Customer_id, bad.Account_Number p_account_number, bad.Account_type p_account_type, sum(bat.Transaction_amount) s_transaction_amount
from bank_account_details bad
join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
join bank_account_transaction bat
on bad.Account_Number=bat.Account_Number
where bad.Account_type='Savings' and bard.Account_type='Credit Card'
group by bad.Account_Number, bard.Customer_id, bard.Account_Number, bard.Account_type
...
select bad.Customer_id, bad.Account_Number p_account_number, bad.Account_type p_accunt_type, bard.Account_type s_account_type, sum(bat.Transaction_amount) as sum_transaction
from bank_account_details bad
join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
join bank_account_transaction bat
...
group by bad.Customer_id, bad.Account_Number, bad.Account_type, bard.Account_type
...
select bad.customer_id, bad.account_number p_account_number,bad.account_type p_account_type, bard.account_number as s_account_number, bard.account_type s_account_type, sum(bat.Transaction_amount) as sum_Transaction_amount
from bank_account_details bad
join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
join bank_account_transaction bat
...
group by bad.account_number, bard.account_number, bard.account_type
...
select bad.customer_id, bad.account_number, bad.account_type p_account_type, bard.account_type s_account_type, sum(bat.Transaction_amount) as rd_Transaction_amount
from bank_account_details bad
join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
join bank_account_transaction bat
on bad.Account_Number=bat.Account_Number
where bard.account_type ='RECURRING DEPOSITS' and date_format(bat.Transaction_Date,'%Y-%m') = '2020-02'
group by bard.account_number, bad.account_number, bard.account_type

# Question 15:
# 15) Display every account's total no of transactions for every year and each month.
  
  select Account_Number, count(*) total_no_of_transactions, year(Transaction_Date), month(Transaction_Date)
...
select bat1.account_number, bat1.transaction_amount, sum(bat2.Transaction_Date), sum(bat1.Transaction_Date)
from bank_account_transaction bat1
join bank_account_transaction bat2
on bat1.Account_Number=bat2.Account_Number
where date_format(bat2.Transaction_Date,'%Y-%m') = '2020-02' and date_format(bat1.Transaction_Date,'%Y-%m') = '2020-01'
...
where bard1.Account_type = 'RECURRING DEPOSITS' and bard2.Account_type = 'Credit Card'


# Question 18:
# 18) Display savings accounts and its corresponding Recurring deposits transactions that are occuring more than 4 times.

select bad.customer_id, bad.account_number, bad.account_type p_account_type, bard.account_type s_account_type
from bank_account_details bad
join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
join bank_account_transaction bat
on bad.Account_Number=bat.Account_Number
where bad.Account_type='Savings' and bard.account_type='RECURRING DEPOSITS'
group by bad.customer_id, bad.account_number, bard.account_type
...
select bad.customer_id, bad.account_number, bad.account_type p_account_type, bard.account_type s_account_type, sum(Transaction_amount), month(bat.Transaction_Date)
from bank_account_details bad
join bank_account_relationship_details bard
on bad.Account_Number=bard.Linking_Account_Number
join bank_account_transaction bat
...
select bad.account_type, bad.Account_Number, count(Transaction_Date)
from bank_account_details bad
join bank_account_transaction bat
on bad.Account_Number=bat.Account_Number
