...
select product, month, count(product) from bank_inventory_pricing where month=1 group by product having count(product)>1
...
select product, month, count(product), purchase_cost, estimated_sale_price
from bank_inventory_pricing
where purchase_cost>coalesce(estimated_sale_price,0)
group by product
...
select distinctrow product, quantity, price, purchase_cost, estimated_sale_price
from bank_inventory_pricing 
...
select product, avg(coalesce(purchase_cost, greatest(price, estimated_sale_price)))
from bank_inventory_pricing
...
select product, avg(coalesce(estimated_sale_price, purchase_cost))
from bank_inventory_pricing
group by product
...
#Finding count of each product in every month
#select product, count(product), month
#from bank_inventory_pricing
